...
async def get_by_user(user_id: int = Path(gt=0), db: Session = Depends(get_db)):
    return crud_review.get_by_user(db=db, user_id=user_id)
...
async def create_review(review_request: ReviewRequest, db: Session = Depends(get_db)):
    return crud_review.create(db=db, obj_request=review_request)
...
async def update_review(review_request: ReviewRequest, review_id: int = Path(gt=0), db: Session = Depends(get_db)):
    crud_review.update(db=db, obj_id=review_id, obj_request=review_request)
...
async def delete_review(review_id: int = Path(gt=0), db: Session = Depends(get_db)):
    crud_review.remove(db=db, obj_id=review_id)
